...
%         b=parm.aggr.b(i_b);
%         w = fx(a,b,i_pt_temp./grid_gen.nxy);
%         %plot(1-parm.aggr.sum*ones(1,numel(i_pt_temp)))
%         %plot(parm.aggr.sum*(1-w));
%         plot(w);
%     end
% end
...
parm.aggr.method='A';
parm.aggr.A = 0:0.1:.5;
parm.n_realisation = parm.par_n*4*numel(parm.aggr.A);

% filename = 'ResCst01';
% parm.aggr.sum = 1;
% parm.aggr.method='cst';
% parm.aggr.w=[0:.1:1];
% parm.n_realisation  = parm.par_n*2*numel(parm.aggr.w);
...
disp('Setup ok, Start')
[Res,~,kern,~, parm, Nscore] =  BSGS(Klog,Sigma,grid_gen,parm);
disp('Run finished')
...
try 
    if strcmp(parm.aggr.method,'AB') || strcmp(parm.aggr.method,'A')
    ...
    elseif strcmp(parm.aggr.method,'cst')
        parfor i_x = 1:numel(parm.aggr.w)
            idx=i_x:numel(parm.aggr.w):parm.n_realisation;
            OF1(i_x) = sqrt(mean(mean(E1(:,idx),2).^2));
            OF2(i_x) = sqrt(mean(mean(E2(:,idx),2).^2));
        end
    else
