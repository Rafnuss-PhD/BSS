...
function [Res, t, kern, k, parm, Nscore, filename] = A2PCo(Prim,Sec,sec,grid_gen,parm)
t.global = tic;
...
sec.x=sec.x(:);sec.y=sec.y(:);sec.d=sec.d(:);
...
elseif parm.n_realisation > 0
    [Res,t_out] = A2PCo_in(Prim, Sec, kern, k, Nscore, grid, parm);
    t.scale = t_out.scale;
    t.cstk = t_out.cstk;
    t.pt = t_out.pt;
    t.krig = t_out.krig;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Res_out,t_out]=A2PCo_in(Prim, Sec, kern, k, Nscore, grid, parm)
