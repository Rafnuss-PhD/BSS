...
if d.job_type==1 % inverse solution
    ...
    if d.mesh_type==4 || d.mesh_type==5 % quadrilateral mesh
        ...
        d.max_iterations        = 5;        % maximum number of iteration
        ...
        d.alpha_aniso           = 160/15;      % anisotropy of the smoothing factor: > 1 for smoother horizontal, alpha_aniso < 1 for smoother
        % vertical models, or alpha_aniso=1 for normal (isotropic) regularisation
        if d.reg_mode==1
            d.alpha_s           = NaN;   % regularisation to the starting model
        end
        
        d.a_wgt                 = .01;   % error variance with var(R) = (a_wgt*a_wgt) + (b_wgt*b_wgt) * (R*R)
        d.b_wgt                 = .02;   % where R is the resistance measured
        if d.patch_size_x==0 && d.patch_size_y==0
            d.param_symbol
        end
...
%% RUN .EXE
copyfile('data_gen/R2/R2_x64.exe','data_gen/R2/IO-file/')
cd(d.filepath); tic;
status =system('R2_x64.exe');
cd('../../..'); disp(['Model run in ' num2str(toc) ' secondes.'])
