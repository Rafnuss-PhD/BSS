...
if parm.neigh
    ...
    dist = sqrt(((sel_g_ini(:,1)-center(1))/krig.range(1)).^2 + ((sel_g_ini(:,2)-center(2))/krig.range(2)).^2);
...
% * *KRIGING*: Find his kringing value in noraml space:
 ...
 a0_C=covardm_perso(sel_g,[Res.x(pt.x) Res.y(pt.y)],krig.model,krig.var,parm.k.cx);
 ab_C=covardm_perso(sel_g,sel_g,krig.model,krig.var,parm.k.cx);
...
pt_krig.s = sum(krig.var) - pt_krig.lambda'*a0_C;
